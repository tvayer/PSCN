    ...
    if (oneswitch)
    {
        for (i = 0; i < numkeys; ++i)
        {
		    ...
		    szb = (group_node*)val[i].hi;
                ...
                fprintf(f,"%ld %ld",val[i].lo,val[i].hi);
	    else
                fprintf(f,"%ld",val[i].lo);
        }
	...
	fprintf(f," %10" COUNTER_FMT_RAW " graphs : ",count);
        for (i = 0; i < numkeys; ++i)
        {
		    ...
		    if (compare_groupnodes(sza,szb))
		    {
                ...
                fprintf(f,"%s=%ld:%ld",ID(ki),val[i].lo,val[i].hi);
	    else
                fprintf(f,"%s=%ld",ID(ki),val[i].lo);
        }
	fprintf(f,"\n");
    }
}
...
main(int argc, char *argv[])
{
    ...
    oneswitch = twoswitch = doflush = Vswitch = FALSE;
